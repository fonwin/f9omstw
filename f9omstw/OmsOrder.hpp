...
public:
   ...
   /// 建立時須注意, 若此時 owner.Last()==nullptr
   /// - 表示要建立的是 owner 第一次異動.
   /// - 包含 owner.Head_ 及之後的 data members、衍生類別, 都處在尚未初始化的狀態.
   OmsOrderRaw* MakeOrderRaw(OmsOrder& owner, const OmsTradingRequest& req);
};
...
class OmsOrderRaw {
   ...
   /// 初始化給 OmsOrder 的首次異動.
   /// - 設定 this->Owner_; this->Request_;
   /// - 為了速度, 此處應盡可能的簡化, 必要的初值應在「預設建構」裡面處理.
   virtual void Initialize(OmsOrder& owner);

   /// - 設定 this->Owner_; this->Request_; this->Prev_; this->UpdSeq_; ...
...
protected:
   ...
   /// 在此預設建構裡面, 應先將一些欄位初始化, 讓 OmsOrderRaw::Initialize(owner) 可以快一些.
   OmsOrderRaw() : Owner_{nullptr}, Request_{nullptr} {
   }
   virtual ~OmsOrderRaw();

public:
   OmsOrder* const             Owner_;
   ...
   /// 請注意: 此時的 owner 還在建構階段.
   /// 此處返回後才會設定 owner.Head_;
   OmsOrderRaw(OmsOrder& owner) : Owner_(&owner), Request_(owner.Initiator_) {
   }
   ...
   /// - 設定(清除)部分欄位: this->UpdateTime_ = fon9::UtcNow();
