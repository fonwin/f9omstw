...
void OmsRequestRunnerInCore::Update(f9fmkt_TradingRequestSt reqst) {
   ...
   // 新單異動, 需同時更新 OrderSt
   const auto curRxKind = this->OrderRaw_->Request().RxKind();
   if (fon9_LIKELY(curRxKind == f9fmkt_RxKind_RequestNew)) {
   __UPDATE_ORDER_ST:;
   ...
   else if (fon9_UNLIKELY(curRxKind == f9fmkt_RxKind_RequestRerun || curRxKind == f9fmkt_RxKind_RequestForceContinue)) {
