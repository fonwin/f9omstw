...
TwsTradingLineFix::SendResult TwsTradingLineFix::SendRequest(f9fmkt::TradingRequest& req) {
   ...
   const OmsTwsRequestIni* iniReq = static_cast<const OmsTwsRequestIni*>(order->Initiator_);
...
__REQUEST_DELETE:
         ...
         const auto* lastOrd = static_cast<const OmsTwsOrderRaw*>(order->Last());
         OmsTwsQty   fixQty;
         if (fon9_LIKELY(rxKind == f9fmkt_RxKind_RequestNew)) {
            ...
            *--pout = static_cast<char>(lastOrd->OType_);
            pout = RevPutStr(pout,
                             f9fix_SPLTAGEQ(TimeInForce) "0" // TimeInForce(0:Day)
                             f9fix_SPLTAGEQ(TwseOrdType));
            fixQty = iniReq->Qty_;
         }
         else if (fon9_LIKELY(rxKind == f9fmkt_RxKind_RequestChgQty)) {
            assert(dynamic_cast<const OmsTwsRequestChg*>(curReq) != nullptr);
            // OrderQty=整股:欲刪數量, 零股:剩餘數量.
            auto reqQty = static_cast<const OmsTwsRequestChg*>(curReq)->Qty_;
            if (reqQty < 0) {
               if (twseApCode == TwseApCode_OddLot) {
               ...
               else if (fon9::signed_cast(fixQty = lastOrd->LeavesQty_ + lastOrd->CumQty_ - reqQty) <= 0) {
         ...
         pout = fon9::Pic9ToStrRev<3>(pout, fixQty);
         ...
         *--pout = '.';
         ...
         pout = RevPutStr(pout,
                          f9fix_SPLTAGEQ(OrdType) "2" // OrdType(2:LimitPrice)
                          f9fix_SPLTAGEQ(Price));
      }
      if (rxKind != f9fmkt_RxKind_RequestNew) {
         // OrigClOrdID.
